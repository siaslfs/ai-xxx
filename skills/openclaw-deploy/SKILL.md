...
description: Deploy and configure OpenClaw gateway on remote macOS machines via SSH. Covers Feishu bot setup, model provider configuration (custom LiteLLM / Anthropic direct), launchctl keepalive, and health monitoring. Use when the user asks to install, configure, or troubleshoot OpenClaw on a remote machine.
---

# OpenClaw 远程部署

在远程 macOS (Mac Mini) 机器上部署 OpenClaw gateway 的完整工作流。

## 前置条件

- 目标机器已安装 OpenClaw（通常在 `/opt/homebrew/bin/openclaw` 或 `~/.local/bin/openclaw`）
- 可通过 SSH 访问目标机器（`sshpass -p '<password>' ssh -o PreferredAuthentications=password,keyboard-interactive user@host`）
- 用户提供：飞书 appId、appSecret，以及以下其一：
...
对于网络不稳定的环境，部署 [healthcheck.sh](healthcheck.sh) 到目标机器：
...
| `permission error resolving sender name` | 飞书 App 缺少 contact 权限 | 发 feishu-scopes.json 给客户配置 |
